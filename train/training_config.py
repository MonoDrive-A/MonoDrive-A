...
class DetectionClassWeightConfig:
    ...
    其中 Agent / Map 各自使用 ``agent_focal_gamma`` / ``map_focal_gamma``。
    ``auto`` 模式下 ``α_t`` 按 RetinaNet 约定映射：前景类为 ``agent_focal_alpha`` /
    ``map_focal_alpha``，none 类为 ``1 - focal_alpha``；``manual`` 模式改由
    ``*_non_none_weight`` / ``*_none_weight`` 指定。
    """

    mode: str
    agent_focal_gamma: float
    map_focal_gamma: float
    agent_focal_alpha: float
    map_focal_alpha: float
    ...
    def __post_init__(self) -> None:
        ...
        for field_name in ("agent_focal_gamma", "map_focal_gamma"):
            value = getattr(self, field_name)
            if value < 0.0:
                raise ValueError(f"{field_name} 不能为负数，实际为 {value}。")
        for field_name in ("agent_focal_alpha", "map_focal_alpha"):
            value = getattr(self, field_name)
            if value < 0.0 or value > 1.0:
                raise ValueError(f"{field_name} 必须位于 [0, 1]，实际为 {value}。")
        if self.mode == "auto":
            if self.agent_focal_alpha in (0.0, 1.0):
                raise ValueError(
                    "auto 模式下 agent_focal_alpha 不能为 0 或 1，否则前景或 none 的 α_t 会为 0。"
                )
            if self.map_focal_alpha in (0.0, 1.0):
                raise ValueError(
                    "auto 模式下 map_focal_alpha 不能为 0 或 1，否则前景或 none 的 α_t 会为 0。"
                )
        for field_name in (
    ...
    def agent_focal_alpha_weights(self) -> tuple[float, float]:
        ...
        return self.agent_focal_alpha, 1.0 - self.agent_focal_alpha
    ...
    def map_focal_alpha_weights(self) -> tuple[float, float]:
        ...
        return self.map_focal_alpha, 1.0 - self.map_focal_alpha
...
def load_training_run_config(
    ...
    return TrainingRunConfig(
        ...
        detection_class_weights=DetectionClassWeightConfig(
            ...
            agent_focal_gamma=_require_float(
                detection_class_weights_config,
                "agent_focal_gamma",
            ),
            map_focal_gamma=_require_float(
                detection_class_weights_config,
                "map_focal_gamma",
            ),
            agent_focal_alpha=_require_float(
                detection_class_weights_config,
                "agent_focal_alpha",
            ),
            map_focal_alpha=_require_float(
                detection_class_weights_config,
                "map_focal_alpha",
            ),
