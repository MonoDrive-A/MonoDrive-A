...
配置路径和输出目录必须是项目内相对路径。实现端只读取和校验配置，不填补配置默认值。检测分类权重模式支持 `auto`、`manual` 和 `disabled`，自动模式需要配置 non-none 组 logits 梯度预算比例。
...
| 日期 | 修改人 | 变更 |
| --- | --- | --- |
| 2026-06-08 | 1os3_Codex | AI 完成：同步自动检测分类 non-none 组 logits 梯度预算配置摘要。 |
