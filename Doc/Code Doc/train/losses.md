...
训练日志会额外记录 Agent / Map 检测分类 CE 的 none 与 non-none 分项（`agent_class_ce_non_none`、`agent_class_ce_none`、`map_class_ce_non_none`、`map_class_ce_none`），不参与总 loss 加权，仅用于监控。`auto` 模式下 non-none 为匹配 query 的前景 Focal + objectness，`none` 为未匹配 query 的 none Focal 并按 `sqrt(N_fg / N_bg)` 缩放后的贡献；Map `auto` 分项同样包含点回归维度 auto scale。
